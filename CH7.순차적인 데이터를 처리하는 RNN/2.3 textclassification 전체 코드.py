...
train_iter, val_iter, test_iter = data.BucketIterator.splits((trainset, valset, testset), batch_sizes=BATCH_SIZE, shuffle=True, repeat=False)

vocab_size = len(TEXT.vocab)
n_classes = 2
print("[TRAIN] : %d \t [TEST] : %d \t [VOCAB] : %d \t [CLASSES] : %d" %(len(train_iter),len(test_iter),vocab_size,n_classes))
